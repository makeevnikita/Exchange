...
def get_coins():
    ...
    if not coins:
        try:
            ...
            give_coins = ReceiveGiveCurrencies.objects.select_related('give', 'receive', 'token_standart').values('give_id',
                                      'give__currency_name',
                                      'give__currency_name_short',
                                      'give__image',
                                      'give__category_payment_method__id').distinct()

            receive_coins = ReceiveGiveCurrencies.objects.select_related('give', 'receive', 'token_standart').values('receive_id',
                                         'receive__currency_name',
                                         'receive__currency_name_short',
                                         'receive__image',
                                         'receive__category_payment_method__id').distinct()
            
            exchange_ways = ReceiveGiveCurrencies.objects.select_related('give', 'receive', 'token_standart').values('give_id',
                                'receive_id')
            
            give_tokens = ReceiveGiveCurrencies.objects.select_related('give', 'receive', 'token_standart').values('give_id',
                                       'give__token_standart__id',
                                       'give__token_standart__token_standart')\
                                        .exclude(give__token_standart__id = 1).distinct()
            
            receive_tokens = ReceiveGiveCurrencies.objects.select_related('give', 'receive', 'token_standart').values('receive_id',
                                          'receive__token_standart__id',
                                          'receive__token_standart__token_standart')\
                                          .exclude(receive__token_standart__id = 1).distinct()

            coins_name_short = []
            for coin in ReceiveGiveCurrencies.objects.select_related('give', 'receive', 'token_standart').values('give__currency_name_short','receive__currency_name_short')\
...
class NetworkAPI:
    ...
    async def get_rate_crypto(cls):
        rate_dict = {}
        coins = [coin for coin in await get_coins()][5]
        for coin in coins:
